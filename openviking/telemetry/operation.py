    ...
    def build(
        ...
        *,
        ...
        if cls._has_metric_prefix("memory", counters, gauges):
            ...
            if cls._has_metric_prefix("memory.admission", counters, gauges):
                memory_summary["admission"] = {
                    "trace": {
                        "total": cls._i(counters.get("memory.admission.trace.total"), 0),
                        "action": cls._counter_suffix_map(
                            counters,
                            "memory.admission.action.",
                        ),
                        "status": cls._counter_suffix_map(
                            counters,
                            "memory.admission.status.",
                        ),
                        "reason": cls._counter_suffix_map(
                            counters,
                            "memory.admission.reason.",
                        ),
                        "redirected": cls._i(
                            counters.get("memory.admission.redirected"),
                            0,
                        ),
                        "candidates_total": cls._i(
                            counters.get("memory.admission.candidates.total"),
                            0,
                        ),
                    },
                }
